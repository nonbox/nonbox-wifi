(function(){
  var CXT, WiFiScanner, execSyncToBuffer, command;
  ...
  CXT = {
    ...
    WiFiLog: function(msg, error) {
      ...
      if (error) {
        return console.error("WiFi: " + msg);
      } else {
        if (this.WiFiControlSettings.debug) {
          return console.log("WiFi: " + msg);
        }
      }
    }
  };
  ...
  module.exports = {
    ...
    configure: function(settings) {
      ...
      if (settings.debug != null) {
        ...
        CXT.WiFiLog("Debug: " + settings.debug);
      }
      if (settings.connectionTimeout != null) {
        ...
        CXT.WiFiLog("Timeout: " + settings.connectionTimeout + "ms");
      }
      if (settings.iface != null) {
        return this.findInterface(settings.iface);
      }
    ...
    findInterface: function(iface) {
      ...
      try {
        ...
        interfaceResults = command.autoFindInterface.call(CXT);
    ...
    scanForWiFi: function(cb) {
      ...
      try {
        ...
        if (process.platform === "linux") {
          networks = command.scanForWiFi.apply(CXT);
    ...
    connectToAP: function(_ap, cb) {
      ...
      try {
        ...
        command.connectToAP.call(CXT, _ap);
    ...
    disconnectIface: function() {
      var _msg, error, error1;
      try {
        command.disconnectIface.call(CXT);
        CXT.WiFiLog("Disconnecting WiFi interface...");
      } catch (error1) {
        _msg = "Couldn't disconnect WiFi interface"
        CXT.WiFiLog(_msg, true);
        return {
          success: false,
          msg: _msg
    ...
    getIfaceState: function() {
      ...
      try {
        interfaceState = command.getIfaceState.call(CXT);
